function [sv,stat0,tk]=ephpos(time,teph,sat,nav,iode,sv) %#ok
%% ModMenz(TimeDelayEPh
...
if sys==glc.SYS_GPS||sys==glc.SYS_GAL||sys==glc.SYS_BDS||sys==glc.SYS_QZS
    %[eph,stat]=searcheph(teph,sat,iode,nav.eph);
...
elseif sys==glc.SYS_GLO
    %[geph,stat]=searchgeph(teph,sat,iode,nav.eph);
