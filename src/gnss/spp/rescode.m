function [v,H,P,vsat,azel,resp,nv,ns,...
    troperr_vect,ionoerr_vect]=rescode(iter,obs,nav,sv,x0,opt)
...
%     [ionoerr,Viono]=iono_cor(glc.IONOOPT_BRDC,obs(i).time,nav,obs(i).sat,pos,azel(i,:));
    ionoerr=0;
    Viono=0;
    if opt.ionoopt==glc.IONOOPT_BRDC
    [ionoerr,Viono]=iono_cor(glc.IONOOPT_BRDC,obs(i).time,nav,obs(i).sat,pos,azel(i,:));
    end
    ionoerr_vect(i)=ionoerr;
    ...
    troperr=0;Vtrop=0;
    if opt.tropopt==glc.TROPOPT_SAAS
    [troperr,Vtrop]=trop_cor(opt.tropopt,obs(i).time,nav,pos,azel(i,:));
    end
    troperr_vect(i)=troperr;
