function [gps_alm, glo_alm] = readyuma(filename)

% [gps_alm, glo_alm] = readyuma(filename);
%
...
% Modified by: Maria Eagen to alter GPS week rollover handling 10/17/2002
% Modified by: Jimmy LaMance 3/30/2004.  Replaced keyboard statement with
% file processing error and intelligent break.  Modified file name
% generation to tolerate one and two digit week numbers as valid entries.
% Copyright (c) 1998-2004 by Constell, Inc.
...
if nargin >= 1
  if isstr(filename)        % if the input is a string
    ...
    GPS_week_max = 1024;    % maximum value of GPS weeks
    ...
    inputfile(1,:) = sprintf('gps%03d.alm',filename);   % GPS almanac
 
    % if 2 output variables were requested, set up the file name for the 
    % GLONASS almanac also
    if nargout == 2
      inputfile(2,:) = sprintf('glo%03d.alm',filename);   % GLONASS almanac
...
for k = 1:alm_files_to_read
    ...
    for i = 1:num_lines_per_almanac
      ...
      if almanac_line == -1           % is there an end of file
        ...
        if any(I_colon) & I_end > 3              % valid almanac line
          ...
          fprintf('Invalid YUMA almanac file %s.  An empty almanac will be returned for this file.\n', ...
              inputfile(I_valid_file(k),:));
          break;
  ...
  if I_valid_file(k) == 1 & read_error == 0     % the almanac should fo in the GPS almanac return
    gps_alm = alm;
  
  elseif I_valid_file(k) == 1  & read_error == 1 % the almanac should fo in the GLONASS almanac return
    gps_alm = [];
    
elseif I_valid_file(k) == 2  & read_error == 0 % the almanac should fo in the GLONASS almanac return
    glo_alm = alm;
  
  elseif I_valid_file(k) == 2  & read_error == 1 % the almanac should fo in the GLONASS almanac return
    glo_alm = [];
...
% Mod. 10/17/2002
% The almanac week field is no longer modified to add rollover weeks
% Weeks are kept in YUMA standard format
% if rollover_flag ~= 0
%  gps_alm(:,13) = gps_alm(:,13) + rollover_flag*1024;
%  if exist('glo_alm')
%     glo_alm(:,13) = glo_alm(:,13) + rollover_flag*1024;
%  end
