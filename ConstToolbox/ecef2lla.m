...
% Hoffman-Wellenhoff, pages 279-282. 5-th Edition.
...
if method == 0                      % use closed form approximation
  e2 = (RE^2 - b^2) / RE^2;         % first numerical eccentricity 
  ePrime2 = (RE^2 - b^2) / b^2;     % second numerical eccentricity 
  ...
  lla(:,1) = atan2(x_ecef(:,3) + ...
                   ePrime2 .* b .* sin(theta).^3, p - e2 .* RE .* cos(theta).^3);
