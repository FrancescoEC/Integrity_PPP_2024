...
%                of the t_start times. Must be 1x2 or 1x3 if t_stop is given.
%                valid GPS_week values are 1-1024
%                valid GPS_sec values are 0-604799
%   t_stop    - stop time in GPS time format (1x2) [GPS_week GPS_sec] (optional),
%                    or (lx3) [GPS_week GPS_sec rollover_flag]
%                default = 600 seconds past start
%   dt        - output interval, seconds (optional), default = 2 sec
%
% Output:
%   t_out     - GPS time vector output [GPS_week, GPS_sec] (nx2) or
%               [GPS_week GPS_sec rollover_flag] (nx3)
...
if matlab_version >= 5.0                        
  ...
  estruct.variable(2).req_dim = [1 2; 902 2; 1 3; 902 3;];
  estruct.variable(2).var = t_start;
  estruct.variable(2).type = 'GPS_TIME';
  
  estruct.variable(3).name = 't_stop';
  estruct.variable(3).req_dim = [1 2; 902 2; 1 3; 902 3;];
...
if size(t_start,2)==3 | size(t_stop,2)==3,
    t_out = ones(total_size,3) * NaN;
else
...
for jjj = 1:num_active_satellites
  ...
  t_out(output_index,2) = t_input';
   ...
   % Add the rollover flag if the input included it
  if size(t_start,2)==3 | size(t_stop,2)==3,
      t_out(output_index,3) = ones(size(output_index,2),1)*t_input_flag;
  end
