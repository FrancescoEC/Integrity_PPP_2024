...
%   t2  - GPS time vector for group #2 objects (nx2) or (nx3) [GPS_week GPS_sec rollover_flag]
%          valid GPS_week values are 1-1024
%          valid GPS_sec values are 0-604799
%          valid inputs for optional rollover_flag are 0-1
...
%          (e.g. ECEF or ECI).
% Output:
%   t_los        - GPS time vector corresponding to the LOS (kx2) [GPS_week
%                   GPS_sec] or (kx3) [GPS_week GPS_sec rollover_flag]
...
struct.variable(3).req_dim = [902 2; 902 3];
...
% Add rollover flag to GPS time for dates preceeding the GPS week rollover
% in Aug 22, 1999
if rollflag<99,
    t_los(:,3) = rollflag;
end
