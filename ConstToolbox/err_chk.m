...
for i = 1:num_to_check                 
  ...
  if isfield(estruct.variable(i),'type')                      
    ...
    elseif strcmp(estruct.variable(i).type,'GPS_TIME')
      % Check that the inputs are within the error bounds
      gps_time_min = [0 0 0];             % min GPS week and seconds and rollover_flag
      gps_time_max = [3640 604800 1];     % max GPS week and seconds and rollover_flag
      ...
      % Find out of bounds for seconds values
      ...
      if size(estruct.variable(i).var,2)==3,
          % Find out of bounds for rollover_flag values
          I_roll = find(estruct.variable(i).var(:,3) < gps_time_min(3) | ...
                   estruct.variable(i).var(:,3) > gps_time_max(3));
                     
        if ~isempty(I_roll)      
            if DEBUG_MODE == 0        
              fprintf('Error from Constellation Toolbox Error Checking.\n');
              fprintf('   %d GPS rollover_flag variable(s) out of %d\n',length(I_roll),...
                       size(estruct.variable(i).var,1));
              fprintf('   were found to be out of bounds in input %s to function %s.\n', ...
                       estruct.variable(i).name, upper(estruct.func_name));
              fprintf('   Valid values are between 0 and 1.\n');
              fprintf('   Type "help %s" for correct units and details.\n', ...
                        lower(estruct.func_name));
              fprintf('   Resulting use of these variables may cause subsequent ');
              fprintf('functions to fail.\n\n');
        ...
        end % if ~isempty(I_roll)
      end % if size(e_struct.variable(i).var,2)==3,
