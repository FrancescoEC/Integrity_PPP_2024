function [fig_han,gga_out,gsv_out,gsa_out] = ...
                      ex_nmea(file_name,plot_flags,edit_data_flag);

% [fig_han,gga_data,gsv_data,gsa_data] = ex_nmea(file_name,plot_flags,edit_flag);
%
...
if generate_gsv_plots == 1 & ~isempty(gsv_out)
  % format the prn/az/el data for the plotazel function
  ...
  if size(gsv_out,1) ~= lg
    if size(gsv_out,1) ~= lg - 1 
      fprintf('Warning from EX_NMEA.  There seems to be data missing from the\n');
...
if generate_gga_plots == 1  & ~isempty(gga_out)
  ...
  sigma_n_text = sprintf('Lat/Lon Positions\nSigma North = %8.2f\nSigma East = %8.2f',sigmas(1),sigmas(2));
  legend(sigma_n_text,0)
